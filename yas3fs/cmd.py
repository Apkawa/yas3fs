...
from .log import logger, CompressedRotatingFileHandler
from .utils import create_dirs, custom_sys_excepthook
from .core import YAS3FS
...
AWS_DEFAULT_REGION environment variable can be used to set the default AWS region."""
    ...
    if options.log:  # Rotate log files at 100MB size
        ...
        if options.log_backup_gzip:
            logHandler = CompressedRotatingFileHandler(options.log, maxBytes=log_size,
                backupCount=options.log_backup_count)
        else:
            logHandler = logging.handlers.RotatingFileHandler(options.log, maxBytes=log_size,
                backupCount=options.log_backup_count)
