...
from .plugins.YAS3FSPlugin import YAS3FSPlugin
from .utils import error_and_exit, create_dirs_for_file, remove_empty_dirs_for_file, get_current_time, \
    get_uid_gid, thread_is_not_alive
        ...
        with self.get_lock(wait_until_cleared_proplist):
            if prop == None:
                ...
                if prop == 'change' and 'invoke_after_change' in self.props:
                    logger.debug(
                        'FSData.props[change] removed, now executing invoke_after_change lambda for: ' + self.path)
    ...
    def wait_until_cleared(self, path, proplist=None, max_retries=10, wait_time=1):
        ...
        for prop in proplist:
            ...
            if not cleared:
                ...
                raise Exception(
                    "Path has not yet been cleared but operation wants to happen on it '%s' '%s'" % (prop, path))
        return True
    ...
    def do_POST(self):
        ...
        if verify_evp.verify_final(signature.decode('base64')):
            ...
            if message_type == 'Notification':
                ...
                self.server.fs.http_subscription = response['ConfirmSubscriptionResponse']['ConfirmSubscriptionResult'][
                    'SubscriptionArn']
...
class PartOfFSData():
    ...
    def read(self, n=-1):
        logger.debug(
            "read '%i' '%s' at '%i' starting from '%i' for '%i'" % (n, self.content, self.pos, self.start, self.length))
        ...
        if n >= 0:
            ...
            if len(s) != n:
                logger.error(
                    "read length not-equal! '%i' '%s' at '%i' starting from '%i' for '%i'  length of return ['%s] " % (
...
class YAS3FS(LoggingMixIn, Operations):
    ...
    def __init__(self, options):
        ...
        self.yas3fs_xattrs = ['user.yas3fs.bucket', 'user.yas3fs.key', 'user.yas3fs.URL', 'user.yas3fs.signedURL',
                              'user.yas3fs.expiration']
        ...
        if self.sqs_queue_name or self.new_queue:
            ...
            self.queue.set_message_class(
                boto.sqs.message.RawMessage)  # There is a bug with the default Message class in boto
            ...
            if purge:
                ...
                with self.cache.get_lock(path):
                    if self.cache.has(path):  # Path may be deleted before I acquire the lock
                        ...
                        data = self.cache.get(path, 'data')
                        full_delete = False
                        if (not data) or (data and (store == '' or data.store == store) and (not data.has('open')) and (
                        not data.has('change'))):
                            if store == '':
                            ...
                            logger.debug("check_cache_size purge: '%s' '%s' KO data? %s open? %s change? %s"
                                         % (store, path, data != None, data and data.has('open'),
    ...
    def download_data(self, path, start, end):
        ...
        while retry:
            ...
            try:
                ...
                path, start, end, thread_name, self.download_retries_num, self.download_retries_sleep,
                retriesAttempted))
        ...
        if debug:
            ...
            logger.debug(
                "download_data done '%s' %i-%i [thread '%s'] elapsed %.6f" % (path, start, end, thread_name, elapsed))
    ...
    def do_cmd_on_s3_now(self, key, pub, action, args, kargs):
        ...
        try:
            if action == 'delete':
                ...
                logger.debug(
                    "Multipart-upload Key Sizes '%s' local: %i remote: %i" % (key, full_size, uploaded_key.size))
                if full_size != uploaded_key.size:
                    logger.error("Multipart-upload Key Sizes do not match for '%s' local: %i remote: %i" % (
    ...
    def do_cmd_on_s3_now_w_retries(self, key, pub, action, args, kargs, retries=1):
        ...
        for tries in range(1, retries + 1):
            ...
            try:
                logger.debug("do_cmd_on_s3_now_w_retries try %s action '%s' key '%s' args '%s' kargs '%s'" % (
        ...
        logger.error(
            "do_cmd_on_s3_now_w_retries FAILED '%s' key '%s' args '%s' kargs '%s'" % (action, key, args, kargs))
    ...
    def read(self, path, length, offset, fh=None):
        ...
        while retry:
            ...
            if retriesAttempted > self.read_retries_num:
                logger.error(
                    "read '%s' '%i' '%i' '%s' max read retries exceeded max: %i sleep: %i retries: %i, raising FuseOSError(errno.EIO) ''" % (
    ...
    def write(self, path, new_data, offset, fh=None):
        ...
        with data.get_lock():
            if not data.content:
                ...
                return 0
            logger.debug("write '%s' '%i' '%i' '%s' '%s' content" % (
            path, len(new_data), offset, fh, data.content.name.decode('utf-8')))
            data.content.seek(offset)
    ...
    def upload_to_s3(self, path, data):
        ...
        logger.debug("multipart test: key '%s' mp-num '%s' st_size '%s' mp-size '%s'" % (
        path, self.multipart_num, attr['st_size'], self.multipart_size))
